...
- [ ] Runner trust is evidenced: runner type, owner, isolation boundary, workspace reuse policy, network egress posture, and whether self-hosted runners are single-tenant and recreated between jobs.
...
- [ ] Release artifacts are promoted from the reviewed build output or linked by hash/provenance to the reviewed build. Rebuilding from a tag at release time requires a reproducibility check before crediting promotion continuity.
...
- OIDC trust policies that grant cloud roles to broad subjects such as all pull requests, all branches, or all repositories without `sub`, `aud`, branch/ref, environment, and workflow constraints.
...
**OIDC evidence gate:** When OIDC or workload identity federation is present, record the effective trust policy, including `aud`, `sub`, repository/org scope, branch/ref restrictions, environment requirements, workflow filename constraints, and whether pull-request contexts from forks can assume the role. Treat "OIDC enabled" as insufficient evidence without these constraints.
...
- Persistent runners that reuse workspace directories, Docker layers, tool caches, or credentials across jobs from different trust levels.
- Ephemeral runners with no workspace reuse and strict OIDC subject claims, which should be distinguished from generic self-hosted runner risk rather than over-reported.
...
**Runner trust evidence gate:** For every runner class used in privileged build, test, signing, publish, or deploy jobs, record whether the runner is hosted or self-hosted, ephemeral or persistent, single-tenant or shared, whether workspace/cache reuse crosses trust boundaries, what secrets/cloud roles are available, and what network egress is permitted.
...
- Release artifacts rebuilt from source/tag instead of promoted from the reviewed build output without a reproducibility check.
- Artifact hash, SBOM, attestation, or signature generated after promotion rather than bound to the original reviewed build.
...
**Artifact promotion evidence gate:** Trace the artifact from reviewed source commit to build output to release/deploy input. Record artifact digest, provenance/attestation ID, signing identity, storage location, promotion job, and whether release jobs consume the same artifact or rebuild it. If release rebuilds are used, require deterministic rebuild evidence before treating the chain as continuous.
...
- **Runner trust:** <runner type, owner, ephemeral/persistent model, workspace/cache reuse, isolation boundary, egress, secrets/cloud role exposure>
- **OIDC subject constraints:** <aud, sub, repo/org, branch/ref, environment, workflow, fork PR restrictions>
- **Artifact promotion continuity:** <source commit, build artifact digest, attestation/signature, storage location, release input, rebuild/reproducibility evidence>
...
- **1.0.1** -- Added runner trust, OIDC subject-constraint, and artifact promotion continuity evidence gates for ephemeral runners, workload identity federation, and release artifact provenance.
